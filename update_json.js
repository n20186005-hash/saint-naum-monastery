...
for (const file of files) {
  ...
  // 2. Intro aboutTitle and aboutText
  if (data.intro) {
    data.intro.aboutTitle = isZh ? '关于彼得罗瓦拉丁堡垒钟塔' : 'About Petrovaradin Fortress Clock Tower';
    data.intro.aboutText = isZh ? 
  ...
  // alsoKnownAs if the user meant that for highlights title or similar
  if (data.intro && data.intro.alsoKnownAs) {
  ...
  // 7. Route Steps (Delete steps 7 and 8, which are indices 6 and 7)
